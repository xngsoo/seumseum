...
- 삭제는 soft delete 하지 않는다. 지출은 수정 모달에서만 지운다.
  휴지통 → 확인 창 → 즉시 삭제, 모달이 닫힌 뒤 목록 화면이 취소 스낵바를 띄운다.
  확인 창은 잘못 눌러 지우는 것을 막고, 스낵바는 확인하고도 마음이 바뀐 경우를 받는다
...
## Screens
- 탭 1 Daily: 상단 날짜·요일·합계, 하단 그날 내역 리스트(금액/내용/카테고리).
  좌우 스와이프로 날짜 이동. 행을 누르면 수정 모달이 열린다.
  목록에서는 순서를 바꾸거나 지우지 않는다 (스와이프가 날짜 이동과 겹친다).
  빈 날짜도 빈 상태 화면을 보여준다
